...
set_var = {"in", "if", "elif", "else", "for", "pass", "return", "not", "def", "is", "and", "or"}
...
def adjustAfterwards(list_code, index1, dic_var, dic_rename, f_debug):
    if list_code[1][0] == 1:
        ...
        if list_code[1][1] == "for":
            ...
            if index1 == 4 and (list_code[4][0] == 323 or list_code[4][0] == 324) and list_code[4][1][1] == "enumerate": 
...
def adjustAfterwards2(list_tmp, dic_var, dic_rename, f_debug):
    ...
    if list_tmp[0] == 299:# or list_tmp[0] == 298: # for:299, while:298
        ...
        if list_tmp[0] == 299:
            ...
            list1 = list_tmp[4]
        checkExp(list_check, list1, set_name, dic_var)
        for str_name in set_name:
...
set_method_bisect = {"bisect_left", "bisect_right", "bisect", "insort_left", "insort_right", "insort"}
...
def get_var3(list1, dic_var, list_tmp, dic_rename):
    ...
    if list_tmp[0] == 272:
        ...
        if list_tmp[2][0] == 36:
            ...
            indentnum = 0
            if list_tmp[1][0] == 323 or list_tmp[1][0] == 324:
            ...
            if list_tmp[3][0] == 1:
                set_timecpx2 = dic_var[str_name2][2]
                set_spacecpx2 = dic_var[str_name2][2]
            elif list_tmp[3][0] == 323 or list_tmp[3][0] == 324:
                
                str_name2, indentnum2, f_ref2 = getIndexDepth(dic_var, list_tmp[3])
                set_timecpx2 = dic_var[str_name2][2]
                set_spacecpx2 = dic_var[str_name2][2]
            elif list_tmp[3][0] == 324:
                set_timecpx2 = list_tmp[3][-1][1]
                set_spacecpx2 = list_tmp[3][-1][2]
            #print(str_name2, str_name)
            set_timecpx = setunion(dic_var[str_name][1], set_timecpx2) 
            set_spacecpx = setunion(dic_var[str_name][2], set_spacecpx2)
            dic_var[str_name] = ["list", set(), set_spacecpx]
        ...
        for index1 in range(1, len(list_tmp)):
            ...
            if list2[0] == 1:
                ...
                dic_var[list2[1]] = [type1[0], set(), type1[2]]
                ...
                if list_tmp[-1][0] == 324:
                    ...
                    str_name_tmp = list2[1]+"*"
                    if list3[2][1][0] == 1:
                        if list3[2][1][1] in dic_var:
                            dic_var[str_name_tmp] = dic_var[list3[2][1][1]]
                        else:
                            dic_var[str_name_tmp] = ["int", set(), set()]
                    elif list3[2][1][0] == 324 or list3[2][1][0] == 323:
                        dic_var[str_name_tmp] = [list3[2][1][-1][0], set(), list3[2][1][-1][2]]
                        
                        if list3[2][1][2][1] == "(" or list3[2][1][2][1] == "[":
                            dic_var[str_name_tmp] = list3[2][1][0][-1]
                        if list3[2][1][2][1] == "{":
                            dic_var[str_name_tmp] = list3[2][1][0][-1]
    ...
    if list_tmp[0] == 312:  
        if list_tmp[2][1] == "in":
            ...
            if list_tmp[3][0] == 323 or list_tmp[3][0] == 324:
                str_name, indentnum, f_ref = getIndexDepth(dic_var, list_tmp[3])
                if str_name in dic_var:
                    if dic_var[str_name][0] == "list":
                        set_timecpx = dic_var[str_name][2]
                        set_spacecpx = dic_var[str_name][2]
                    else: # set
                        set_timecpx = set()
                        set_spacecpx = set()
                    return ["exp:in*", set_timecpx, set_spacecpx]
                else:
                    return ["exp:in*", set(), set()]
    ...
    if list_tmp[0] == 323 or list_tmp[0] == 324:
        
        if list_tmp[1][1] == "len":
            return ["int", list_tmp[2][-1][1], list_tmp[2][-1][2]]
        ...
        if list_tmp[1][1] == "set" or list_tmp[1][1] == "dict":
            # 
            if list_tmp[2][2][0] == 323 or list_tmp[2][2][0] == 324:
        ...
        if list_tmp[1][1] == "sorted":
            ...
            return ["list", set_time_cpx, list_tmp[2][2][-1][2]]
        ...
        f_ref = False
        if list_tmp[2][1][0] == 9: 
            set_timecpx = list_tmp[2][-1][1]
            set_spacecpx = list_tmp[2][-1][2]
            
            str_name, indentnum, f_ref = getIndexDepth(dic_var, list_tmp)
            if f_ref is True: 
                if str_name in dic_var:
                    set_timecpx = setunion(set_timecpx, dic_var[str_name][1])
                    set_spacecpx = setunion(set_spacecpx, dic_var[str_name][2])
                    str_type = dic_var[str_name][0]# "exp:ref*"
                    return [str_type, set_timecpx, set_spacecpx]
        ...
        if list_tmp[2+indentnum][1][0] == 23:
            if f_ref is False and indentnum == 0:
                str_name = list_tmp[1][1]
...
def getIndexDepth(dic_var, list_tmp):
    ...
    for index2 in range(2, len(list_tmp)):
        if list_tmp[index2][0] == 326 or list_tmp[index2][0] == 327:
...
def checkExp(list_name, list_tmp, set_varname, dic_var):
    if len(list_tmp) == 3:
        if type(list_tmp[1]) is list:
            return checkExp(list_name, list_tmp[1], set_varname, dic_var)
        else:
        ...
        for i in range(1, len(list_tmp)):
            ...
            if list_tmp[i][1] == "":
                continue
            #
            f_list, f_ok = checkExp(list_name, list_tmp[i], set_varname, dic_var)
            ...
            if type(f_ok) is list:
                ...
                if str_name in dic_var and dic_var[str_name][0] == f_ok[1]:
                    set_varname.add(str_name) # 変数名を格納する
                return False, False
        return False, False
    return False, False
